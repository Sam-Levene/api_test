...
describe("Site Check", async function() {
    ...
    it("ii.    Logs in as a Registered User (I WILL FAIL)", async function () {
    ...
    it("xvi.  Obtains a token from Payment API (I WILL FAIL)", async function() {
        // Payment
    ...
    it ("xvii. Uses the Payment API token to get an actual Cybersourse token from Cybersource with card information (I WILL FAIL)", async function() {
        return PaymentService.getActualCSToken(cybersourceToken).then((newTokenResponse) => {
            expect(HttpResponseCodes.isBadRequest(newTokenResponse.statusCode)).to.equal(true);
    ...
    it("xviii. Makes a payment using the new Cybersource token and deal information (I WILL FAIL)", async function() {
    ...
    it("xix.  Retrieves Customer Vouchers (I WILL FAIL)", async function () {
        // Voucher
        ...
        return VoucherService.getVouchersForUser(wowcherUser).then((voucherResponse) => {
            ...
            expect(HttpResponseCodes.isUnauthorized(voucherResponse.statusCode)).to.equal(true);
        });
    }).timeout(10000);
});