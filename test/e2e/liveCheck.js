...
describe("Site Check", async function() {
    ...
    it("i.     Registers a User", async function () {
        ...
        if( ! universalStorage.exists('userCreateAfterIterations')) {
            // This will happen only the first iteration
            universalStorage.store('userCreateAfterIterations', '1');
        }
        else {
            let userCreateAfterIterations = parseInt(universalStorage.fetch('userCreateAfterIterations'));
            ...
            if( userCreateAfterIterations++ > iterationLimit) {
                ...
                universalStorage.store('userCreateAfterIterations', userCreateAfterIterations.toString());
            }
        }

        // If user exists don't create.  Retrieve details from US
        if(( universalStorage.exists('username'))) {
            // email / password will be used for test 2 )
            wowcherUser.email = universalStorage.fetch('username');
            wowcherUser.password = universalStorage.fetch('password');
            authToken = universalStorage.fetch('authToken');
            customerToken = universalStorage.fetch('customerToken');
            ...
            return ApiServiceUser.registerUser(wowcherUser).then((response) => {
                ...
                universalStorage.store('username', wowcherUser.email);
                universalStorage.store('password', wowcherUser.password);
                universalStorage.store('authToken', authToken);
                universalStorage.store('customerToken', customerToken);
    ...
    it("vi.    Looks up address using postcode", async function () {
